...
def get_actions_dict(phases):
    ...
    for p in phases:
        if "y" not in p.state:
            phases_dict[i] = (p.state, p.duration)
            i += 1
    return phases_dict
...
class Environment:
    ...
    def __init__(self, run_with_gui, concat_lane=None):
        ...
        self.actions = get_actions_dict(self.get_phases())
    ...
    def set_action(self, action, state_as_matrix=False, ai=True):
        ...
        self.do_stats()
        self.set_phase(pred_phase, self.min_duration if ai else self.actions[action][1],
